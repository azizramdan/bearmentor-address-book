(() => {
  ...
  const id = url.get('id')
  const currentLabel = (new Label()).show(id)

  document.getElementById('label-id').value = id

  if (!currentLabel) {
    document.getElementById('main').innerHTML = /* html */`<div class="h-[88dvh]">Label not found</div>`
  ...
  function initEvents() {
    document.getElementById('contacts-section').addEventListener('click', (event) => {
  ...
  function renderContactCounter() {
    document.getElementById('label-name').textContent = currentLabel.name
