(() => {
  ...
  document.getElementById('back-button').setAttribute('href', `/person/?id=${id}`)
  ...
  document.getElementById('edit-contact-form').addEventListener('submit', submitForm)
      ...
      .map(label => /* html */`
        <div>
          <input
            ...
            value="${label.id}"
            class="peer hidden"
            ${currentContact.labels.includes(label.id) ? 'checked' : ''}
          />
          <label
  ...
  function fillFormValues() {
    ...
    document.getElementById('middleName').value = currentContact.middleName
    ...
    document.getElementById('jobTitle').value = currentContact.jobTitle
    document.getElementById('notes').value = currentContact.notes

    currentContact.emails.forEach(email => addEmailForm(email))
    currentContact.phones.forEach(phone => addPhoneForm(phone))
    currentContact.addresses.forEach(address => addAddressForm(address))
  }

  function addEmailForm(data = {}) {
    const formContainerElement = document.getElementById('emails-form-container')
    const id = Date.now() + Number.parseInt(Math.random() * 1000)
            ...
            <div class="bg-white rounded-lg text-left">
              <div class="relative bg-inherit">
                <select
                  ...
                  class="peer bg-transparent h-10 w-full rounded-sm placeholder-transparent ring-1 focus:ring-2 px-2 ring-gray-500 focus:ring-blue-600 focus:outline-none focus:border-rose-600"
                >
                  ${window.COUNTRIES.map(country => `<option value="${country}" ${data?.country === country ? 'selected' : ''}>${country}</option>`).join('')}
                </select>
                <label
  ...
  function submitForm(event) {
    ...
    window.location.href = `/person/?id=${id}`
  }
})()
